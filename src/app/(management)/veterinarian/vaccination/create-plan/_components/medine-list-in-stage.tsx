...
const MedicineListInStage = ({
  ...
  medicineInStageProp: MedicineInStage;
  updateMedicines: any;
}) => {
  const { isOpen, onOpen, onOpenChange, onClose } = useDisclosure();
  const [medicinesInStage, setMedicinesInStage] =
    React.useState<MedicineInStage>(medicineInStageProp);
  ...
  React.useEffect(() => {
    if (Object.entries(selectedMedicine).length > 0) {
      ...
      if (
        pluck("medicineId", medicinesInStage.medicines).includes(
          selectedMedicine.id
        )
      ) {
        newMedicines = medicinesInStage.medicines.map((x: MedicineEachStage) =>
          x.medicineId === selectedMedicine.id
            ? {
                ...x,
                portionEachPig:
                  x.portionEachPig + Number(selectedMedicine.portionEachPig),
              }
            : { ...x }
        );
      }else{
  ...
  const onRemove = (medicineId: string) => {
    const newMedicines = medicinesInStage.medicines.filter(
        ...
        <TableHeader columns={columns}>
          {(column) => (
            ...
            >
              {column.name}
              {column.uid === "actions" && (
                <Plus className="cursor-pointer" onClick={onOpen} />
              )}
            </TableColumn>
          )}
        </TableHeader>
        <TableBody emptyContent={"Chưa chọn thuốc cho giai đoạn này"}>
          {medicinesInStage.medicines.map(
            (item: MedicineEachStage, index: number) => (
              <TableRow key={item?.medicineId ?? 0}>
                {(columnKey) => (
