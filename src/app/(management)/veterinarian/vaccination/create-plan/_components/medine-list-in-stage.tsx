"use client";
import { Button, Spinner, Table, TableBody, TableCell, TableColumn, TableHeader, TableRow, Tooltip, useDisclosure } from "@nextui-org/react";
      ...
      if (pluck("medicineId", medicinesInStage.medicines).includes(selectedMedicine.id)) {
        newMedicines = medicinesInStage.medicines.map((x: MedicineEachStage) =>
          x.medicineId === selectedMedicine.id
            ? {
                ...x,
                portionEachPig: x.portionEachPig ?? 1 + Number(selectedMedicine.portionEachPig),
              }
            : { ...x }
        );
      } else {
  ...
  return (
    <div>
      <Table>
        ...
        <TableBody isLoading={loading} loadingContent={<Spinner label="Loading..." />} emptyContent={"Chưa chọn thuốc cho giai đoạn này"}>
