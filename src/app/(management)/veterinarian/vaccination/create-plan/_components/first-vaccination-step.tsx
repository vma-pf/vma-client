...
const FirstVaccinationStep = ({ setStep, setVaccinationPlanFirstStepResult }: any) => {
  ...
  const checkStep1Completed = (): void => {
    const validateStages = stages.filter((x: VaccinationStageProps) => x.title === "" || x.timeSpan === "" || x.applyStageTime === "");
      ...
      <div className="container mx-auto">
        <form onSubmit={handleSubmit(handleSubmitForm)}>
          <div className="flex justify-end">
            <Button color="primary" variant="solid" isDisabled={errors && Object.keys(errors).length > 0} type="submit" endContent={<ChevronRight />}>
              Tiếp theo
            </Button>
          </div>
          ...
          <Card className="my-4">
            <CardBody>
              <div className="mb-2 flex flex-row justify-between">
                <p className="text-2xl font-semibold">Giai đoạn</p>
                  ...
                  <div key={stage.id} className="flex flex-row justify-between">
                    ...
                    <div className="flex flex-row items-start">
                      <span className="text-lg text-danger cursor-pointer active:opacity-50">
                        {stages.length > 1 && <Trash2Icon onClick={() => onDeleteStage(stage)} />}
                      </span>
                    </div>
                  </div>
                );
