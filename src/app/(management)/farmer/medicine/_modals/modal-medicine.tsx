...
const ModalMedicine = ({ isOpen, onOpenChange, updateId, context, setSubmitDone }: any) => {
  ...
  const getDataById = async () => {
    setLoading(true);
    try {
      const response = await medicineService.getMedicineById(updateId);
  ...
  const handleSubmitForm = async (data: CreateMedicineRequest | UpdateMedicineRequest) => {
    try {
      setLoading(true);
      const response = context === "create" ? await medicineService.createMedicine(data) : await medicineService.updateMedicine(updateId, data);
