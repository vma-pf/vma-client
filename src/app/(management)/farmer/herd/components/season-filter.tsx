...
const SeasonFilter = ({ setSelectedHerd }: {
  ...
  const fetchHerd = async () => {
    try {
      const response: ResponseObjectList<HerdInfo> = await apiRequest.getHerd(1, 500);
      if (response.isSuccess) {
        setHerds(response.data.data);
        setSelectedHerd(response.data.data.filter((herd) => herd.id === selectedValue).length > 0 ? response.data.data.filter((herd) => herd.id === selectedValue)[0] : response.data.data[0]);
