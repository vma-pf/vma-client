...
const RenderStage = ({ stage }: RenderStageProps) => {
  ...
  const fetchMedicine = async (stageId: string) => {
    try {
      setIsLoading(true);
      const response: ResponseObject<any> = await treatmentStageService.getMedicineInStage(stageId ?? "");
                  ...
                  <HoverCardContent align="start" className="w-96 bg-gray-100">
                    {medicine.medicine && medicine.medicine != null ? (
                      <div>
                        ...
                        <p>
                          <strong>Khối lượng:</strong> {medicine.medicine.netWeight} {medicine.medicine.unit}
                        </p>
                      </div>
                    ) : (
