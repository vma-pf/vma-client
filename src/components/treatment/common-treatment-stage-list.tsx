...
const RenderStage = ({ stage }: RenderStageProps) => {
  ...
  const [selectedMedicineDetail, setSelectedMedicineDetail] =
    React.useState<Medicine | null>(null);
...
const CommonTreatmentStageList = ({
  stages,
}: {
  ...
  if (!stages || stages.length === 0) {
    return (
      <div className="text-center mt-4">Không có giai đoạn điều trị nào</div>
    );
  }

  const currentTreatmentStages = stages.filter(
    (stage) =>
      new Date(stage.applyStageTime) >=
  ...
  if (currentTreatmentStages.length === 0) {
    return (
      <div className="text-center mt-4">
        Chưa có giai đoạn điều trị nào đến hạn
      </div>
    );
  }
